...
def main(folder: Path):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        t1 = executor.submit(parser.scan, folder.resolve())
        t2 = executor.submit(parser.scan, folder.resolve())
        t3 = executor.submit(parser.scan, folder.resolve())
        t4 = executor.submit(parser.scan, folder.resolve())

        print(f'Thread 1: {t1.__dict__["_state"]}''\n'
              f'Thread 2: {t2.__dict__["_state"]}''\n'
              f'Thread 3: {t3.__dict__["_state"]}''\n'
              f'Thread 4: {t4.__dict__["_state"]}')
